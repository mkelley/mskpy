...
class NEATM(SurfaceRadiation):
    ...
    def __init__(
        self, D, Ap, eta=1.0, epsilon=0.95, G=0.15, phaseint=None, tol=1e-3, **kwargs
    ):
        self.D = D.to(u.km)
        self.Ap = Ap
        self.eta = eta
    ...
    def fluxd(self, geom, wave, unit=u.Jy):
        ...
        fluxd *= (
            (self.epsilon * (self.D / geom["delta"]) ** 2 / pi / 2.0).decompose().value
        )  # W/m^2/Hz

        fluxd = fluxd * u.Unit("W / (m2 Hz)")
        fluxd = fluxd.to(unit, u.spectral_density(wave))
        if len(fluxd) == 1:
            return fluxd[0]
        else:
            return fluxd
    ...
    def T0(self, rh):
        ...
        T0 = (((1.0 - self.A) * Fsun) / abs(self.eta) / self.epsilon / sigma) ** 0.25
        return T0 * u.K
...
class DAp(SurfaceRadiation):
    ...
    def fluxd(self, geom, wave, unit=u.Unit("W / (m2 um)")):
        ...
        if unit != fsca.unit:
            fsca = fsca.to(unit, u.spectral_density(wave))
