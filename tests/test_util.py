...
class TestUtilMathmatical():
    def test_haversine(self):
        th = np.arange(0, 2 * pi)
        assert np.allclose(archav(hav(th)), th)

    def test_cartesian(self):
        a = [0, 1, 2]
        b = [-1, 0]
        c = np.array([[0, -1], [0, 0], [1, -1], [1, 0], [2, -1], [2, 0]])
        assert cartesian(a, b) == c

    def test_davint(self):
        x = np.arange(0, 2 * pi)
        y = np.sin(x)
        assert davint(x, y, 0, 2 * pi) == 0

    def test_deriv(self):
        a = np.arange(10)
        assert np.allclose(deriv(a), np.ones(10))

    def test_gaussian(self):
        assert gaussian(0, 0, 1) == 1 / np.sqrt(2 * pi)
        assert (gaussian(2 * np.sqrt(2 * np.log(2)) / 2.0, 0, 1)
                == gaussian(0, 0, 2))
        assert gaussian(1, 1, 1) == 1 / np.sqrt(2 * pi)

    def test_rotmat(self):
        r = np.array([1, 0]) * rotmat(pi / 2).squeeze()
        assert np.allclose(r, [0, 1])
        r = np.array([1, 0]) * rotmat(-pi / 2).squeeze()
        assert np.allclose(r, [0, -1])
        r = np.array([1, 0]) * rotmat(pi).squeeze()
        assert np.allclose(r, [-1, 0])
        r = np.array([1, 0]) * rotmat(2 * pi).squeeze()
        assert np.allclose(r, [1, 0])

class TestSearchingSorting()
    def test_between(self):
        a = np.arange(100, int)
        assert between(a, [5, 8]) == np.array([5, 6, 7, 8])
        assert between(a, [5, 8], closed=False) == np.array([6, 7])

    def test_cmp_leading_num(self):
        a = '1P 103P 88P asdf'.split()
        a.sort(cmp_leading_num)
        assert a == '1P 88P 103P asdf'.split()

    def test_groupby(self):
        lists = (list('abcdef'), range(6))
        key = [3, 3, 100, 0, 0, 100]
        grouped = groupby(key, *lists)
        assert key[100] == (['c', 'f'], [2, 5])

    def test_nearest(self):
        a = np.logspace(-1, 2, 7)
        n = nearest(a, np.log(20))
        assert n == 2

    def test_takefrom(self):
        r = takefrom((list('asdf'), list('jkl;')), [3, 2])
        assert r == (['f', 'd'], [';', 'l'])
    ...
    def test_whist(self):
        ...
        y = x**2
        w = 1.0 / x
        h = whist(x, y, w, errors=False, bins=[0, 2.5, 5.0])
